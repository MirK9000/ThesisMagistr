% Подключение файла с настройками и подключенными пакетами
\input{settings/preamble.tex}

\begin{document}

% Пустая первая страница вместо титульного листа для сохранения нумерации
\thispagestyle{empty}
\mbox{}
\newpage

% TODO: Сюда будет вставлено Задание на ВКР после его получения (например, \includepdf[pages={1-2}]{extra/Zadanie.pdf})

% Содержание, Введение
\input{chapters/introduction.tex}

% Подключение глав
\input{chapters/chapter_1.tex}
\input{chapters/chapter_2.tex}
\input{chapters/chapter_3.tex}

% Заключение временно исключено из текущей сборки
% \input{chapters/conclusion.tex}

% Библиографический список
\input{chapters/bibliography.tex}

% Приложение (может быть пустым)
\input{chapters/appendix.tex}

\end{document}
